    ...
    <nav class="container mx-auto px-4 py-4">
      <div class="flex items-center justify-between">
        ...
        <router-link to="/" class="flex items-center space-x-2 group">
          <div
            class="w-10 h-10 bg-gradient-primary rounded-lg flex items-center justify-center transform group-hover:rotate-12 transition-transform duration-300"
          >
            <svg
        ...
        <div class="hidden md:flex items-center space-x-4">
          ...
          <div v-if="isLoggedIn" class="relative">
            ...
            >
              <div
                class="w-10 h-10 bg-gradient-primary rounded-full flex items-center justify-center text-white font-semibold"
              >
                {{ userInitial }}
              </div>
              <svg
  ...
  setup() {
    ...
    const handleSearch = () => {
      if (searchQuery.value.trim()) {
        // TODO: 实现搜索功能,您可以在这里添加搜索逻辑
        router.push({ path: "/", query: { keyword: searchQuery.value } });
        searchQuery.value = "";
      }
    };
