      ...
      <div class="container mx-auto px-4">
        <div class="max-w-5xl mx-auto mb-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-3xl font-bold text-gray-900">写回复</h1>
              <p class="text-sm text-gray-500 mt-1">
                <template v-if="replyToUsername">
                  回复 <span class="text-primary-600">@{{ replyToUsername }}</span>
                </template>
                <template v-else>
                  回复评论
                </template>
...
export default {
  ...
  setup() {
    ...
    const loadReplyData = () => {
      // Get reply data from route state
      const replyData = history.state?.replyData
      if (replyData) {
        commentId.value = replyData.commentId
        postId.value = replyData.postId
        replyToUserId.value = replyData.replyToUserId || null
        replyToUsername.value = replyData.replyToUsername || ''
      } else {
        // If no reply data, redirect back
        alert('回复数据丢失')
        router.back()
      }
    }
    ...
    const handleSubmit = async () => {
      ...
      try {
        await createReply(commentId.value, content.value, replyToUserId.value, replyToUsername.value)
        ...
        if (err.response?.status === 403) {
          const errorMsg = typeof err.response?.data === 'string'
            ? err.response.data
            : err.response?.data?.message || '操作被拒绝'
