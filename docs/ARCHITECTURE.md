...
| 管理员控制器 | `controller/AdminController.java` | 管理后台 API 端点 |
| 安全配置 | `config/SecurityConfig.java` | URL级别权限配置 |
| 用户详情服务 | `security/CustomUserDetailsService.java` | 加载用户角色信息 |

### 前端组件

| 组件 | 路径 | 说明 |
|------|------|------|
| 管理后台API | `api/admin.js` | 管理后台 API 封装 |
| 管理仪表盘 | `views/admin/AdminDashboard.vue` | 管理后台主界面 |
