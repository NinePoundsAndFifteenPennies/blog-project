...
在创建或更新文章时，可以通过 `categoryId` 字段指定分类：
...
- `categoryId` 为可选字段
- 如果提供了无效的 `categoryId`，将返回 404 错误
- 更新文章时，如果不提供 `categoryId`，则会移除文章的分类
...
4. **删除保护**：系统会阻止删除仍在使用中的分类，确保数据完整性
