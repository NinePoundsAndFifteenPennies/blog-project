...
### 1. 功能模块差异
* **未实现功能**: 原始方案中规划的 **评论 (Comment)**、**分类 (Category)** 和 **标签 (Tag)** 功能及其相关API在本期开发中并未实现，数据库中也仅创建了`users`和`posts`两张核心表，前端还没开始开发。
...
修复了个人中心草稿箱不显示草稿文章的问题。

**📖 查看完整修复文档**: [FIX_INDEX.md](./FIX_INDEX.md)
...
## 下一步任务
接下来完成网页端页面框架的构建（各个页面，组件，样式，交互等），并且与后端crud的各个接口对其，实现一个相对完整的网页程序（注册登录，增删查改文章，分页显示文章）。